...
void loop() {
  ...
  if (!addButtonValue) { //Check if the button is pressed
      ledBlinkTime += 1; //add pwm value
    }
  if (!deButtonValue) { //Check if the button is pressed
      ledBlinkTime -= 1; //de pwm value
    }
  ledBlinkTime = constrain(ledBlinkTime, 0, 30000);
  digitalWrite(ledPin, HIGH); //Write the values
  delayMicroseconds(ledBlinkTime*2);  //Pause the program for ledBlinkTime
  digitalWrite(ledPin, LOW); //Write the values
  delayMicroseconds(ledBlinkTime*2);  //Pauses for the time ledBlinkTime
